 ...
 *
 *		$Id: Disk.php 29031 2012-03-23 02:44:25Z songlixin $
 */
...
class Cloud_Service_Disk {
	...
	public function __construct() {
		...
		$this->_baseUrl = $_G['siteurl'] . 'portal.php?';
	}
	...
	public function saveTask($attach, $openid) {
		...
		if (!$_G['uid']) {
			throw new Exception('noLogin', '30001');
		}
		if (!$openid) {
			throw new Exception('noOpenid', '30002');
		}
	...
	public function getAttachment() {
		...
		$sId = $_G['setting']['my_siteid'];
		$ts = $_GET['ts'];
		$taskId = dintval($_GET['taskid']);
		$verifyCode = trim($_GET['verifycode']);
		if ($sId != $_GET['s_id']) {
			throw new Exception('sIdError', '30004');
		}
		if ($_G['timestamp'] - $ts > 86400) {
			throw new Exception('downloadTimeOut', '30005');
		}
		$params = array(
			'taskid' => $taskId,
			'verifycode' => $verifyCode,
		...
		if ($sig != md5(sprintf('%s|%s|%s', $str, $sKey, $ts))) {
			throw new Exception('sigError', '30003');
		}
		$task = C::t('#qqconnect#connect_disktask')->fetch($taskId);
		if (!$task) {
			throw new Exception('noTask', '30006');
		}
		if ($verifyCode != $task['verifycode']) {
			throw new Exception('verifyError', '30009');
		}
		if ($task['status'] == 1) {
			throw new Exception('attachmentHadDownloaded', '30008');
		}

		$attach = C::t('forum_attachment_n')->fetch('aid:' . $task['aid'], $task['aid']);
		if (!$attach) {
			throw new Exception('noAttachment', '30007');
		}
