 ...
 *
 *      $Id: search.class.php 28980 2012-03-21 06:34:25Z zhouxiaobo $
 */
		...
		foreach ($fids as $v) {
			...
			if (!$forum_recwords) {
				...
				foreach($forum_recwords as $key => $word) {
					$forum_recwords[$key]['url'] = $searchparams['url'] . '?' . $srchotquery . '&q=' . urlencode($word['word']) . '&source=word.hotopic_if.'.$key.'&keywordType=recommend&hwfId=' . $v;
				}
