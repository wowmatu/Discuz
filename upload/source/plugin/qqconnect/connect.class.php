 ...
 *
 *		$Id: connect.class.php 29033 2012-03-23 03:47:47Z monkey $
 */
...
class plugin_qqconnect extends plugin_qqconnect_base {
	...
	function _viewthread_bottom_output() {
		...
		if ($_G['cookie']['connect_last_sync_t']) {
			return;
		}

		if($page == 1 && $postlist[$_G['forum_firstpid']]['invisible'] == 0) {
			$jsurl = '';
			if($_G['uid'] && $_G['setting']['connect']['t']['reply'] && !$thread['closed'] && !$rushreply && getstatus($_G['forum_thread']['status'], 14)) {
