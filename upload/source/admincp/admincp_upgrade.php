 ...
 *
 *      $Id: admincp_upgrade.php 28991 2012-03-22 01:18:10Z svn_project_zhangjie $
 */
...
if($operation == 'patch' || $operation == 'cross') {
	...
	if($step == 1) {
		...
		if($upgradeinfo['isupdatedb']) {
			...
			cpmsg('upgrade_file_successful', $_G['siteurl'].'install/update.php?step=prepare&from='.rawurlencode($_G['siteurl'].ADMINSCRIPT.'?action=upgrade&operation='.$operation.'&version='.$version.'&release='.$release.'&step=5'), '', array('upgradeurl' => upgradeinformation(4)));
		}
